...
const Dashboard = () => {
  ...
  return (
    ...
    {/* Main Content */}
    <main
      className={`flex-1 ${darkMode ? 'bg-[#030917]' : 'bg-white'} transition-all duration-300 ease-in-out ${
        isSidebarOpen ? 'blur-sm pointer-events-none md:pointer-events-auto' : ''
      } md:ml-64`}
    >
      {/* Header Section */}
      <header
    ...
    <UserMenu />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4 lg:p-8">
        {/* Proxy Card */}
        <div
          className={`rounded-lg p-6 shadow-md flex border flex-col items-center ${
            darkMode ? 'bg-[#1a1b1f] border-gray-700' : 'bg-white border-gray-200'
          }`}
        >
        ...
        <div
          className={`rounded-lg p-6 shadow-md flex flex-col items-center border ${
            darkMode ? 'bg-[#1a1b1f] border-gray-700' : 'bg-gray-100 border-gray-300'
          }`}
        >
